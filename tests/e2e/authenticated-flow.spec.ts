...
test('account session and favorites persist across sign-out', async ({ page }) => {
  ...
  const email = 'fixture-user@wcsmontpellier.invalid'
  const password = 'WcsDemo-2026!'
  ...
  const existingFavorite = page.getByRole('button', { name: 'Retirer des favoris' }).first()
  if (await existingFavorite.isVisible()) {
